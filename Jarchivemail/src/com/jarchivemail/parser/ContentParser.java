 ...
 * @Date 2012-2-26
 */
public class ContentParser extends Mail {

	public ContentParser(LineReader lr, int startLine) {
		super(lr, startLine);
	}

	public ContentParser(LineReader lr, int startLine, int endLine) {
		super(lr, startLine, endLine);
		...
		try {
			lineReader.setLineNr(startLine);
			while ((line = lineReader.readLine()) != null) {
				if (line.regionMatches(false, 0, "From ", 0, 5)) {
					curLine--;
					break;
				}
				curLine = lineReader.getLineNr() - 1;
								...
								if (st.hasMoreTokens()) {
									...
									if (s.equals("UUENCODE")) {
										...
										contentEncodings.addElement(new String(
												"UUENCODE"));
										lineReader.setLineNr(curLine);
										contentFileNames
												.addElement(extractUUFileName(lineReader
														.readLine()));
									} else {
										contentTypes.addElement(new String(
												"TEXT/PLAIN"));
			...
			lineReader.setLineNr(curLine + 1);
		} catch (IOException e) {
		}
	}
