...
import com.eams.mboxparser.util.Base64;
import com.eams.mboxparser.util.HTMLParserUtil;
import com.eams.mboxparser.util.QuotedPrintable;
import com.eams.mboxparser.util.Uuencode;
...
public class MboxParser {
	...
	public MboxParser(String directory, String fileName) {
		this(new File(directory + fileName));
	}

	public MboxParser(File f) {
		String line;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				if (line.regionMatches(false, 0, "From ", 0, 5)) {
					...
					MailParser mh = new MailParser(br, curLine + 1);
	...
	public String getMessageContent(int index) {
		...
		if (knownContents[k - 1].equals("TEXT/HTML")) {
			HTMLParserUtil fh = new HTMLParserUtil(outBuf.toString());
			...
			for (int j = 0; j < numContents; j++) {
				if (fileNames[j] == null) {
					if ((types[j] != null && types[j]
						...
						MailParser mh = new MailParser(br, contentStart,
								contentEnd);
