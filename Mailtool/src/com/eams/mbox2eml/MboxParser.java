...
/**
 * 
 * @Description Mbox (mailbox) parser. This version returns the headers for the
 *              first email via metadata, which means headers from subsequent
 *              emails will be lost.
...
public class MboxParser {
	...
	private static final String EMAIL_FROMLINE_METADATA = EMAIL_HEADER_METADATA_PREFIX
			+ "from";
	...
	public void parse(InputStream stream, ContentHandler handler,
		...
		try {
			...
			throw new Exception("US-ASCII is not supported!", e);
		}
		...
		// "From "
		for (String curLine = reader.readLine(); curLine != null; curLine = reader
				.readLine()) {
			...
			// We'd also want this for charset handling, where content isn't
			// 7-bit
			// ascii.
			case IN_CONTENT:
				if (newMessage) {
	...
	private void endMessage(XHTMLContentHandler xhtml, boolean inQuote)
			throws SAXException {
		if (inQuote) {
			xhtml.endElement("q");
		}
	...
	private void saveHeaderInMetadata(int numEmails, Metadata metadata,
			String curLine) {
