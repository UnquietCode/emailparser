...
public class LogsController {
	...
	public String list(final Invocation inv) {
		...
		final List<Log> logs = (pageIndex <= 1) ? this.logDAO.find(
				user.getName(), PER_PAGE_LIMIT) : this.logDAO.find(
				user.getName(), pageIndex, PER_PAGE_LIMIT);

		System.out.println(pageIndex);

		// 构造一个page对象，第1个参数是当前页，第2个参数是该页最大记录数，第3个是页码上的连接地址
		Page page = new Page(pageIndex, PER_PAGE_LIMIT, "logs?pageIndex="
				+ pageIndex);
		...
		return "logs?pageIndex=" + pageIndex;
	}
